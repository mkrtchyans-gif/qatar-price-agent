...
# Все города пишутся на один лист
SHEET_NAME = os.environ.get("SHEET_NAME", "Цены").strip()

ROUTES = {
    "Boston": {"origin": "MOW", "destination": "BOS", "city_ru": "Бостон"},
    "Los Angeles": {"origin": "MOW", "destination": "LAX", "city_ru": "Лос-Анджелес"},
}
...
def previous_total(ws, city_ru):
    """Последняя цена по ЭТОМУ городу. Строки разных городов чередуются."""
    for row in reversed(ws.get_all_values()[1:]):
        if len(row) > 4 and row[1].strip() == city_ru and row[4]:
            try:
                return float(str(row[4]).replace(",", ".").replace(" ", ""))
            except ValueError:
                continue
    return None
...
def main():
    ...
    ws.append_row(
        [datetime.now(timezone.utc).strftime("%d.%m.%Y"), city_ru,
         ru(depart), ru(ret), total],
    ...
    if prev is not None and prev > 0:
        ...
        if drop_pct >= ALERT_DROP_PCT:
            telegram_send(
                ...
                f"<b>{city_ru}</b> ({origin} \u2192 {destination})\n"
                f"Было: {prev:.0f} USD\n"
    ...
    if summary_path:
        with open(summary_path, "a", encoding="utf-8") as fh:
            fh.write(
                f"## {city_ru}: {total} USD за {PASSENGERS} чел.\n\n"
                f"**Вылет {ru(depart)} → обратно {ru(ret)}**\n\n"
