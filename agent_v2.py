...
def fill_airport(page, labels, value):
    field = first_visible([
        page.get_by_placeholder(re.compile("|".join(labels), re.I)),
        page.locator("input[aria-label]").filter(has=page.locator("xpath=..")),
    ])
    if field is None:
    ...
    if not click_first(page, [
        page.get_by_text(re.compile(fr"\b{re.escape(value)}\b", re.I)),
        ...
        click_first(page, [page.locator('[role="listbox"] >> text=' + value)], timeout=1500)
    page.wait_for_timeout(300)


def set_passengers_and_cabin(page):
    # Open passenger/class editor.
    opened = click_first(page, [
        page.get_by_role("button", name=re.compile(r"Passengers / Class|Passengers", re.I)),
        page.get_by_text(re.compile(r"^Passengers / Class$|^Passengers$", re.I)),
    ], timeout=2000)

    # If the editor is already open, or opening succeeded, set counts.
    if opened or visible(page.get_by_text(re.compile(r"Adults", re.I)).first):
        # Default is typically 1 adult; add one adult.
        click_first(page, [
            page.get_by_role("button", name=re.compile(r"increase adult count", re.I)),
            page.get_by_text(re.compile(r"\+increase adult count", re.I)),
        ], timeout=1500)
        # Age 12 => Teenager (12-15) on Qatar's current booking form.
        click_first(page, [
            page.get_by_role("button", name=re.compile(r"increase teenager count", re.I)),
            page.get_by_text(re.compile(r"\+increase teenager count", re.I)),
        ], timeout=1500)
        # Age 4 => Child (2-11).
        click_first(page, [
            page.get_by_role("button", name=re.compile(r"increase child count", re.I)),
            page.get_by_text(re.compile(r"\+increase child count", re.I)),
        ], timeout=1500)

        click_first(page, [
            page.get_by_text(re.compile(r"^Economy$", re.I)),
            page.get_by_role("button", name=re.compile(r"Economy", re.I)),
        ], timeout=1200)

        # Confirm passenger editor if such button exists.
        click_first(page, [
            page.get_by_role("button", name=re.compile(r"^Confirm$", re.I)),
            page.get_by_text(re.compile(r"^Confirm$", re.I)),
        ], timeout=1200)


def date_input_by_role(page, return_field=False):
    patterns = [
    ...
    for p in patterns:
        for loc in [
            page.get_by_placeholder(re.compile(p, re.I)),
            page.locator(f'input[aria-label*="{p}"]'),
        ]:
            try:
...
def select_calendar_day(page, target: date):
    ...
    for _ in range(6):
        ...
        clicked = click_first(page, [
            page.get_by_role("button", name=re.compile(r"^Next$|Next month|next", re.I)),
    ...
    candidates = [
        ...
        page.locator(f'[data-date="{iso}"]'),
        page.get_by_role("button", name=re.compile(fr"^{target.day}$")),
        page.get_by_text(re.compile(fr"^{target.day}$")),
    ]
...
def set_date(page, target: date, return_field=False):
...
def run_city(browser, city, origin):
    pairs = [(o, r) for o in OUTBOUND_DATES for r in RETURN_DATES]
    best = None

    # Separate contexts/pages per worker. Keeps each individual search bounded while reducing runtime.
    def worker(pair):
        o, r = pair
    ...
    if best:
        ...
        return {
            ...
            "status": f"OK ({24 - failures}/24 searches)",
        }
    ...
    return {
        ...
        "outbound_checked": f"{OUTBOUND_DATES[0].isoformat()}..{OUTBOUND_DATES[-1].isoformat()}",
        "return_checked": f"{RETURN_DATES[0].isoformat()}..{RETURN_DATES[-1].isoformat()}",
        "status": f"ERROR: no valid price ({failures}/24 searches failed)",
    }
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            result = run_city(browser, CITY, TRIPS[CITY]["origin"])
        finally:
            browser.close()
